/**
 * Formatting and parsing of Persian integer numbers.
 *
 ...
 * @throws {Error} If the value is not an integer or if minDigits is not a non-negative integer.
 */
export function formatInteger(value: number, minDigits = 0): string {
    if (!Number.isInteger(value)) {
        throw new Error('Value must be an integer.');
    }
    if (!Number.isInteger(minDigits) || minDigits < 0) {
        throw new Error('Minimum digits must be a non-negative integer.');
    }
...
export function parseInteger(str: string): number {
    ...
    if (input.length === 0) {
        throw new Error('Cannot parse empty string as an integer number.');
    }
